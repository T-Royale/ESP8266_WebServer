...
#define Prueba_CSS \
    ...
    "p { color: #333333; font-size: 3.5em; text-align: center; background: rgba(255, 0, 0, 0.2); padding: 20px; border-radius: 15px; box-shadow: 0 0 15px rgba(0, 0, 0, 0.5); transition: background 1s, color 1s; }" \
    "p:hover { background: rgba(0, 255, 0, 0.2); color: #4B0082; transform: scale(1.3); }" \
    "@keyframes bg-animation { 0% { background-color: #f4f4f4; } 100% { background-color: #b2ebf2; } }" \
    "@keyframes bounce { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-20px); } }" \
    "</style>" \
    "</head>" \
    "<body>" \
    "<h1>Hola!</h1>" \
    "<p>Esta es mi pagina web tras algo de CSS</p>" \
    "</body>" \
    "</html>"
